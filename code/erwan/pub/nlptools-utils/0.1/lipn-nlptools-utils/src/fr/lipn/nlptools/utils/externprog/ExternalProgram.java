...
public class ExternalProgram {
	...
	public int run(WriterFeeder stdinFeeder, ReaderConsumer stdoutConsumer, ReaderConsumer stderrConsumer) throws ExternalProgramException {
		...
		outputConsumerThread = new Thread(new InputStreamConsumer(externalProcess.getInputStream(), lastStdoutConsumer, this, STDOUT_ID));
		outputConsumerThread.start();
		errorConsumerThread = new Thread(new InputStreamConsumer(externalProcess.getErrorStream(), lastStderrConsumer, this, STDERR_ID));
		errorConsumerThread.start();
		int status = -1;
		processThread = new Thread(new WaiterThread(externalProcess, this, DEFAULT_ID));
		processThread.start();
		if (lastStdinFeeder != null) {
			inputFeederThread = new Thread(new OutputStreamFeeder(externalProcess.getOutputStream(), lastStdinFeeder, this, STDIN_ID));
		...
		if (waitForStreamThreads) {  // if any thread has already been interrupted, they should all have been interrupted (thus there will be no infinite waiting)
			log(Level.INFO, "Waiting for stream threads to end.");
	...
	protected void waitForThread(Thread t, int id) {
		if ((t != null) && t.isAlive()) {
			log(Level.INFO, STREAM_THREAD_NAME[id]+" is alive, waiting...");
			try {
				t.join();
	...
	protected void interruptAllThreads() {
		...
		if ((inputFeederThread != null) && inputFeederThread.isAlive()) {
			log(Level.INFO, "input feeder thread was alive.");
			inputFeederThread.interrupt();
		}
		if (outputConsumerThread.isAlive()) {
			log(Level.INFO, "output consumer thread was alive.");
			outputConsumerThread.interrupt();
		}
		if (errorConsumerThread.isAlive()) {
			log(Level.INFO, "error consumer thread was alive.");
			errorConsumerThread.interrupt();
		}
		if (processThread.isAlive()) {
			log(Level.INFO, "main process waiter thread was alive.");
			processThread.interrupt();
		}
	}

	protected synchronized void informInterrupt(int id, Exception e) {
		log(Level.WARNING, "Receiving an exception sent by "+STREAM_THREAD_NAME[id]+" (only the first one will be thrown): ", e);
		if (firstException == null) {
			firstException = new ExternalProgramException("Error sent by "+STREAM_THREAD_NAME[id]+" (only first error is reported).", e);
			interruptAllThreads();
		}
	}
	...
	protected class OutputStreamFeeder implements Runnable {
		...
		public void run() {
			...
			try {
				...
				log(Level.INFO, "Stream has been written normally in "+STREAM_THREAD_NAME[id]);
			} catch(Exception e) {
				log(Level.WARNING, "Exception caught and transmitted in "+STREAM_THREAD_NAME[id]+".");
				caller.informInterrupt(id, e);
			}
