...
//			String id = data.get(ID_TAGNAME);  // ignored because this id seems useless: it is never used as a reference, and there are never two occs with the same id
			...
			//System.err.println("DEBUG Looking for mapping for "+start+" (sentenceId="+sentenceId+")");
			ae.setGenericAttributes(o, 
					tokenIndexMapping.get(sentenceId).get(start).getBegin(), 
					tokenIndexMapping.get(sentenceId).get(end).getEnd(), 
