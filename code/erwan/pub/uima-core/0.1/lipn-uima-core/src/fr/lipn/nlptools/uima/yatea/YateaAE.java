...
//				not used if user-defined config file
	...
	public void process(JCas aJCas) throws AnalysisEngineProcessException {
		...
		if (!YATEA_LANGUAGE_MAP.containsKey(language)) {
			throw new AnalysisEngineProcessException(LipnExternalProgramGenericAnnotator.LIPN_MESSAGE_DIGEST, "invalid_language", new Object[]{language, AE_NAME, YATEA_LANGUAGE_MAP.keySet().toString()});
		}

		if (customConfigFile == null) {
			try {
			// check that language-specific data dir exists (if no user-defined config file is used)
			if (!new File(linguisticDataDir.getCanonicalPath()+DIR_SEPARATOR+YATEA_LANGUAGE_MAP.get(language)).isDirectory()) {
		...
		try {
			...
			config.write("  language = "+YATEA_LANGUAGE_MAP.get(language)+LINE_SEPARATOR);
	    	...
	    	WriterFeeder writerFeeder = new TokenByLineWriterFeeder(new POSLemmaPairInterpretationTokenPOSLemmaIterator(aJCas, true, true, false), LipnExternalProgramGenericAnnotator.LINE_SEPARATOR, TreeTaggerAE.TREETAGGER_COLUMN_SEPARATOR, TreeTaggerAE.TREETAGGER_UNKNOWN_LEMMA_TAG);
			writerFeeder.feedWriter(input);
		} catch (java.io.IOException e) { // a bit simplified, but normally all kind of I/O errors concern writing temp file
	...
	public static ArrayList<HashMap<Integer, Annotation>> initTokenIndexMapping(JCas aJCas) throws AnalysisEngineProcessException {
		...
		while (iter.hasNext()) {
			...
			Integer end = index;
			index++;
			//System.out.println("Writing mapping from start "+start+"->"+token.getBegin()+" and end "+end+"->"+token.getEnd()+" for sentence "+sentenceId);
