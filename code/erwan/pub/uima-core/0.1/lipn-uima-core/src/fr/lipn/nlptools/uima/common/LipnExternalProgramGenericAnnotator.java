...
public abstract class LipnExternalProgramGenericAnnotator extends JCasAnnotator_ImplBase {
	 ...
	 * @param programExecRelPath The name of the program (may be a path relative to the "program directory", e.g. "bin/myprog")
	 ...
	 */
	protected int runProgram(JCas aJCas, ExternalProgram.WriterFeeder stdin, ExternalProgram.ReaderConsumer stdout, boolean debugUseStringStreams, boolean debugWriteStreamsToLog, boolean checkStderr) throws AnalysisEngineProcessException {
	 ...
	 * @throws AnalysisEngineProcessException if exit code is not 0 or stderr is not empty, or if an exception is raised at a lower level.
	 */
	protected int runProgram(JCas aJCas, ExternalProgram.WriterFeeder stdin, ExternalProgram.ReaderConsumer stdout, boolean checkStderr) throws AnalysisEngineProcessException {
		return runProgram(aJCas, stdin, stdout, DEBUG_USE_STRING_STREAMS, DEBUG_WRITE_STREAMS_TO_LOG, checkStderr);
	}

	protected int runProgram(JCas aJCas, ExternalProgram.WriterFeeder stdin, ExternalProgram.ReaderConsumer stdout) throws AnalysisEngineProcessException {
