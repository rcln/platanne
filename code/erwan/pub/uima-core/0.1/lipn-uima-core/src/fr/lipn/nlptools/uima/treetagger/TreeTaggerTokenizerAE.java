...
public class TreeTaggerTokenizerAE extends LipnExternalProgramGenericAnnotator {
	...
	public void initialize(UimaContext context) throws ResourceInitializationException {
		...
		if (context.getConfigParameterValue("DontSplitTotkensFile") != null) {
		...
		if (context.getConfigParameterValue("Language") != null) {
			language = (String) context.getConfigParameterValue("Language");
			if (!TT_TOKENIZER_LANGUAGE_MAP.containsKey(language)) {
				throw new ResourceInitializationException(LipnExternalProgramGenericAnnotator.LIPN_MESSAGE_DIGEST, "invalid_language", new Object[]{language, EXTERNAL_PROGRAM_NAME, TT_TOKENIZER_LANGUAGE_MAP.keySet().toString()});
			}
		}
	...
	public void process(JCas aJCas)  throws AnalysisEngineProcessException  {
		...
		language = ensureLanguage(language, aJCas);
		if (!TT_TOKENIZER_LANGUAGE_MAP.containsKey(language)) {
		...
		try {
			if (language != null) {
				command.add(TT_TOKENIZER_LANGUAGE_MAP.get(language));
			}
	...
	protected class TTTokenizerAnnotReader implements ExternalProgram.ReaderConsumer {
	...
	protected class TTTokenizerConsumer implements AlignerConsumer {
		...
		public void receiveTokenAndAnnotation(String annotatedToken, String inputToken, String annotation, String separator, long startPos, long endPos) throws AlignmentException {
			...
			if (annotation != null) { //should never happen 
				...
				t.setBegin((int) startPos);
				t.setEnd((int) endPos);
				t.setComponentId(TT_TOKENIZER_COMPONENT_ID);
				t.setTypeId(TT_TOKENIZER_COMPONENT_ID);
				t.setConfidence(TT_TOKENIZER_CONFIDENCE);
