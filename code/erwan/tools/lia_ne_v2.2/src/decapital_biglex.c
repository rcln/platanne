...
for(nb=0;fgets(ch,TailleLigne,stdin);nb++)
 ...
 if (pt)
  {
  if ((!strcmp(pt,"A"))&&(strstr(ch+strlen(pt)+1,"PREPADE")))
   {
   /* HACK: FRED0710 on ne rajoute pas le 'a' sans accent en tant que PREPADE .. */
   }
  else
   {
   printf("%s ",decapital_string(pt));
   printf("%s",ch+strlen(pt)+1);
   }
  }
 }
exit(0);
