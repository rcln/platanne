...
currDir=$(pwd)
url="http://pageperso.lif.univ-mrs.fr/~frederic.bechet/software/lia_tagg.v1.1.tgz"
#url="http://pageperso.lif.univ-mrs.fr/~frederic.bechet/download/lia_tagg.tgz"
testOnly=0

function printHelp {
    echo  >/dev/stderr
    echo "This script will try to install LIA TAGG, and will probably fail." >/dev/stderr
    echo "You can at least download the tgz archive before, and provide the location with -a" >/dev/stderr
    echo  >/dev/stderr
    echo "Syntax: $0 [options]" >/dev/stderr
    echo "  Options:" >/dev/stderr
    echo "    -h help " >/dev/stderr
    echo "    -a <tar gz archive> " >/dev/stderr
