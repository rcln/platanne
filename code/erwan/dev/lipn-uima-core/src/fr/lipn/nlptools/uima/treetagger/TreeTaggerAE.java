...
/**
 * This AE is a wrapper for the TreeTagger Part-of-speech tagger. Works for a lot of different languages.
 * TreeTagger outputs in the same time the POS tag and the lemma. Moreover <code>Sentence</code> tags are also written in the CAS by
 * this AE, since this information is also computed by TreeTagger. 
 * A lot of parameters are provided, please see TreeTagger documentation for more details.<br/>
 * 
 * This AE always runs TreeTagger in order to return the probability associated with each POS tag. By default only
 * the most likely POS-lemma pair is written in the CAS, but this behaviour can be changed using <code>MultiTagsProbThreshold</code>.<br/>
 * 
 * If the lemma label is "unknown" in TreeTagger output, then it is NOT written in the CAS. That means that further components
 * must be aware that <code>Lemma</code> annotations are provided for every token.<br/>
 * 
 * 
 * @author moreau
 *
...
public class TreeTaggerAE extends LipnExternalProgramGenericAnnotator {
	 ...
	 * 
	 */
	private static final String TREETAGGER_INVALID_CHARS_REGEXP_TT4J = "\\n\\t\\r\\u200E\\u200F\\u2028\\u2029";
	private static final String TREETAGGER_INVALID_CHARS_REGEXP_OTHERS = "\\0240\\0205\\0376";
	public static final String TREETAGGER_INVALID_CHARS_REGEXP = "["+TREETAGGER_INVALID_CHARS_REGEXP_TT4J+TREETAGGER_INVALID_CHARS_REGEXP_OTHERS+"]+";
	...
	public void process(JCas aJCas) throws AnalysisEngineProcessException {
		...
		String language = ensureLanguage(lang, aJCas, TREETAGGER_LANGUAGE_MAP);
		...
		try {
			if (parameterFile == null) {
				...
				parameterFile = new File(getProgramDir().getCanonicalPath()+DIR_SEPARATOR+PARAMETER_FILE_PREFIX+language+(encoding=="UTF-8"?PARAMETER_FILE_SUFFIX_UTF8:PARAMETER_FILE_SUFFIX_LATIN1));
				getContext().getLogger().log(Level.INFO, "Looking for parameter file for language '"+language+"' encoded as '"+encoding+"', using '"+encoding+"' (file is '"+parameterFile.getCanonicalPath()+"').");
				if (TRY_OTHER_ENCODING_IF_PARAM_FILE_DOES_NOT_EXIST && (!parameterFile.exists())) {
					getContext().getLogger().log(Level.INFO, parameterFile.getCanonicalPath()+" not found, still trying...");
					File tryOtherEncParamFile = new File(getProgramDir().getCanonicalPath()+DIR_SEPARATOR+PARAMETER_FILE_PREFIX+language+(encoding!="UTF-8"?PARAMETER_FILE_SUFFIX_UTF8:PARAMETER_FILE_SUFFIX_LATIN1));
					if (tryOtherEncParamFile.exists()) {
						getContext().getLogger().log(Level.INFO, "No parameter file for language '"+language+"' encoded as '"+encoding+"', using '"+CONVERSE_ENCODING_MAP.get(encoding)+"' (file is '"+tryOtherEncParamFile.getCanonicalPath()+"').");
						encoding = CONVERSE_ENCODING_MAP.get(encoding);
						parameterFile = tryOtherEncParamFile;
					} else {
						getContext().getLogger().log(Level.INFO, tryOtherEncParamFile.getCanonicalPath()+" not found, still trying...");						
					}
				}
				if (!parameterFile.exists()) {
					getContext().getLogger().log(Level.INFO, parameterFile.getCanonicalPath()+" not found.");
		...
		WriterFeeder ttWriterFeeder = new TokenByLineWriterFeeder(new BasicTokenPOSLemmaIterator(aJCas, null, false, false, false, new String[]{ TREETAGGER_INVALID_CHARS_REGEXP }, null, true, null, tokenConstraint, null, null), TREETAGGER_COLUMN_SEPARATOR);
		runProgram(aJCas, ttWriterFeeder, new TreeTaggerAnnotReader(aJCas, tokenConstraint));
	...
	protected class TreeTaggerAnnotReader implements ReaderConsumer {

		JCas aJCas;
		FSMatchConstraint tokenConstraint;

		public TreeTaggerAnnotReader(JCas aJCas, FSMatchConstraint tokenConstraint) {
			this.aJCas = aJCas;
		...
		public void consumeReader(Reader reader) throws ExternalProgramException {
			try {
				...
				Iterator<PositionedText> tokenIterator = new BasicPositionedTextTokenIterator(aJCas, new String[]{ TREETAGGER_INVALID_CHARS_REGEXP }, null, true, null, tokenConstraint);
	...
	protected class TreeTaggerConsumer implements AlignerConsumer {
		...
		public void closeConsumer() throws AlignmentException {
			...
			if ((currentSentenceStart != -1) && (lastTokenEnd != -1)) {
				...
				getContext().getLogger().log(Level.FINEST,"new Sentence (found when closing) : "+currentSentenceStart+"-"+lastTokenEnd+".");
		...
		public void receiveTokenAndAnnotation(String annotatedToken, String inputToken, String annotation, String separator, long startPos, long endPos) throws AlignmentException {
			...
			if (annotation != null) {
				...
				for (int i = 0; i < nbVersions; i++) {
					...
					if (nbVersions > 1) {
						synchronized (aJCas) {
							interp = new POSLemmaPairInterpretation(aJCas);
