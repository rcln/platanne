...
public abstract class GenericTokenIterator<T> implements Iterator<T> {
	 ...
	 */
	public GenericTokenIterator(JCas aJCas, Annotation range, int start, int end, boolean iterateSentences, int tokenType, int sentenceType, String[] regexps, String[] replaceWith, boolean threadSafe, FSMatchConstraint tokenConstraint, FSMatchConstraint sentenceConstraint) {
		super();
		this.aJCas = aJCas;
		...
		if (iterateSentences) {
			sentenceIterator = FSIteratorFactory.createFSIterator(aJCas, sentenceType, range, start, end, threadSafe, sentenceConstraint);
			...
			tokenIterator = FSIteratorFactory.createFSIterator(aJCas, tokenType, range, start, end, threadSafe, tokenConstraint);
		}
		index = 0;
	}
