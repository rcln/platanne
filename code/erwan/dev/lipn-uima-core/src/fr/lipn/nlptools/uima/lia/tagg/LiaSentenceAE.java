...
/**
 * This AE is a wrapper for the LIA sentence tokenizer. for French and English.
 * Notice that this program tends to add unnecessary sentences marks at the end of a document.
 * 
 * @author moreau
 *
 */
public class LiaSentenceAE extends LipnLiaExternalProgramGenericAnnotator {
	...
	public void process(JCas aJCas) throws AnalysisEngineProcessException {
		...
		WriterFeeder writerFeeder = new TokenByLineWriterFeeder( new LiaTokenPOSLemmaIterator(aJCas, false, false, false, null, tokenConstraint, null, null),LipnLiaExternalProgramGenericAnnotator.LIA_TAGG_TAGS_SEPARATOR); 
		runProgram(aJCas, writerFeeder, new LiaSentenceAnnotatedReader(aJCas, tokenConstraint));
	}

	
	protected class LiaSentenceAnnotatedReader implements ReaderConsumer {

		JCas aJCas;
		FSMatchConstraint tokenConstraint;
		
		public LiaSentenceAnnotatedReader(JCas aJCas, FSMatchConstraint tokenConstraint) {
			this.aJCas = aJCas;
		...
		public void consumeReader(Reader reader) throws ExternalProgramException {
			try {
				...
				aligner.align(new TokenIteratorInputReader(new LiaPositionedTextTokenIterator(aJCas, null, tokenConstraint),
		...
		public void receiveTokenAndAnnotation(String annotatedToken, String inputToken, String annotation, String separator, long startPos, long endPos) throws AlignmentException {
			...
			if (annotation != null) {
				if (annotation.equals(LipnLiaExternalProgramGenericAnnotator.LIA_TAGG_SENTENCE_OPEN_TAG)) {
					...
					if (startSentence != -1) {
						if (startSentence != endPos) { // because for some unknown reason the LIA sentence tokenizer tends to add some empty sentences 
							synchronized (aJCas) {     // at the end of the document.
