...
/**
 * this AE is a wrapper for the LIA tokenizer (for French and English).
 * Important: the LIA tokenizer groups together any sequence of words which is considered
 * as a self-contained expression (e.g. "as well as", but also more questionable cases). As
 * a consequence these sequences are considered as a single token, and are also annotated
 * by a <code>Cleaned</code> annotation containing the replacement sequence (usually the same
 * words with underscores instead of whitespaces).
 * 
 * @author moreau
 *
 */
