...
public abstract class LipnLiaExternalProgramGenericAnnotator extends LipnExternalProgramGenericAnnotator {
	...
	protected ExternalProgram initProgram(JCas aJCas, File liaTaggDir, File liaNeDir) throws AnalysisEngineProcessException {
		...
		String normalizedlanguage = ensureLanguage(lang, aJCas, null);  // 1st step : checks lgge is defined, one way or another
		setResourcesNames(normalizedlanguage, liaNeDir);  // needs the "normalized" version of language
		String language = ensureLanguage(normalizedlanguage, aJCas, LIA_LANGUAGE_MAP);  // 2nd step: checks lgge is valid and converts to "LIA" version
		...
		try {
			...
			env.put(LIA_TAGG_LANG_ENVVAR, language);
			if (liaNeDir != null) {
	...
	protected void setResourcesNames(String lang, File liaNeDir) throws AnalysisEngineProcessException {
		try {
			...
			if (lang.equals("fr")) {
				...
				throw new AnalysisEngineProcessException(LIPN_MESSAGE_DIGEST, "invalid_language", new Object[]{getUserFriendlyAEName(), lang, LIA_LANGUAGE_MAP.keySet().toString()});
			}
			if (liaNeDir != null) {
