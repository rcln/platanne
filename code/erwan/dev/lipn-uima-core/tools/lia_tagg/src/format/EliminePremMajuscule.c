...
/* on traite a part le cas : [A partir du ....]
   ...
   PLUS MAINTENANT !!!! FRED 0703

   EUH, DE NOUVEAU OUI !!!! FRED 0904

   re-EUH, sauf option contraire !!
*/

if ((!IF_no_a)&&((ch[0]=='A')&&(ch[1]==' '))) printf("à %s",ch+2);
else
...
if (argc<2)
 {
 fprintf(stderr,"Syntaxe : %s [-h] <lexi compile> [-no_a]\n",argv[0]);
 exit(0);
 }
...
if (!strcmp(argv[1],"-h"))
 ...
 \t Entree : stdin pour le texte et 1er parametre pour le dico\n\
 \t          l'option -no_a permet de ne pas modifier les 'a' de debut de phrase\n\
 \t Sortie : stdout\n\n\
...
if ((argc>2)&&(!strcmp(argv[2],"-no_a"))) IF_no_a=1; else IF_no_a=0;
