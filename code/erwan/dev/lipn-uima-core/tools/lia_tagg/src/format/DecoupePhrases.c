...
if (ch[0]=='\n') return;
for (i=0;ch[i];i++)
 if (!strncmp(ch+i,"</s>",4)) { printf("</s>\n"); if (ch[i+4]=='\n') i+=4; else i+=3; AddStart=1; NotClosed=0; }
 else
  {
  if (AddStart) { printf("<s>\n"); NotClosed=1; }
  printf("%c",ch[i]);
  AddStart=0;
  }
}
...
void TraiteLigne(ch)
 char *ch;
{
static int i_phrase=0,taille=0,last_saut=-1,just_open=1;
int i,n,l;
char *pt;

 for(pt=strtok(ch," \t");pt;pt=strtok(NULL," \t\n"))  /* careful, we jump over \n only after the first token !! */
  {
  if (0) { for(i=0;i<TailleFenetre;i++) fprintf(stderr,"\nF(%d)=[%s]\n",i,Fenetre[i]);  fprintf(stderr,"NEW: pt=[%s]\n",pt); }
  i_phrase++;
  i_phrase=i_phrase%TailleFenetre;
  for(i=0;i<TailleFenetre-1;i++) Fenetre[i]=Fenetre[i+1];
  Fenetre[i]=Phrase[i_phrase];
  for(n=0;(pt[n])&&(pt[n]!='\n');n++) Fenetre[TailleFenetre-1][n]=pt[n];
  Fenetre[TailleFenetre-1][n]='\0';

  if (MotSeparateur(Fenetre[SpotFenetre])) TraiteFinPhrase();
  if (Fenetre[SpotFenetre][0])
   {
   my_printf(Fenetre[SpotFenetre]);
   ...
   if ((1)&&(Fenetre[SpotFenetre][0]=='\0')) /* 1 saut de ligne, on ferma la phrase */
    {
    if ((just_open==0)&&(!AddStart)&&(NotClosed)) { printf("\n</s>\n"); AddStart=1; NotClosed=0; } 
    }
  }
}
